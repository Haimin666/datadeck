...
from server.deps import get_admin_user, get_required_user
...
async def create_provider(
    ...
    try:
        ...
        await log_operation(db, current_user.id, "model_provider.create", f"provider_id={provider.provider_id}")
        await db.commit()
...
async def update_provider(
    ...
    try:
        ...
        await db.commit()
        await _refresh_model_cache()
        await log_operation(db, current_user.id, "model_provider.update", f"provider_id={provider.provider_id}")
        await db.commit()
        return {"success": True, "data": provider.to_dict()}
...
async def delete_provider(
    ...
    await db.commit()
    await _refresh_model_cache()
    await log_operation(db, current_user.id, "model_provider.delete", f"provider_id={provider_id}")
    await db.commit()
    return {"success": True}
