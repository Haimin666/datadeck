...
from server.services.model_providers.builtin import BUILTIN_PROVIDERS
from server.services.model_providers.repository import (
...
from server.models import ModelProvider
...
async def ensure_builtin_model_providers_in_db(db: AsyncSession) -> None:
    ...
    for provider_def in BUILTIN_PROVIDERS:
        ...
        if existing_provider:
            if existing_provider.provider_type is None and provider_def.get("provider_type"):
                existing_provider.provider_type = provider_def["provider_type"]
            if not existing_provider.capabilities and provider_def.get("capabilities"):
                existing_provider.capabilities = provider_def["capabilities"]
...
async def test_model_status_by_spec(spec: str) -> dict:
    """根据 spec 测试模型连接状态（datadeck 简化版：httpx 直测，不引入 embed/rerank 栈）。"""
    ...
    model_type = getattr(info, "model_type", "chat") or "chat"
    try:
        async with session_context() as db:
            if model_type != "chat":
                # embedding/rerank：只验证供应商连通性（list models）
                provider = await get_model_provider_by_id(db, info.provider_id)
                endpoint = None
                headers = {}
                if provider is not None:
                    endpoint = provider.models_endpoint or provider.base_url
                    api_key = resolve_api_key(provider)
                ...
                if endpoint:
                    ...
                    return {
                        ...
                        "message": "连接正常" if ok else f"HTTP {resp.status_code}",
                        "model_type": model_type,
                    }
                return {"spec": spec, "status": "unavailable", "message": "供应商未配置端点", "model_type": model_type}

            # chat：最小 completion 请求
            ...
            model_id = info.model_id or (spec.split(":", 1)[-1] if ":" in spec else spec)
            ...
            try:
                ...
                response = await model.ainvoke([{"role": "user", "content": "Say 1"}])
                content = getattr(response, "content", "")
                if response and (content or getattr(response, "additional_kwargs", {}).get("reasoning_content")):
                    return {"spec": spec, "status": "available", "message": "连接正常", "model_type": model_type}
                return {"spec": spec, "status": "unavailable", "message": "响应无效", "model_type": model_type}
            finally:
        ...
        return {"spec": spec, "status": "error", "message": str(e), "model_type": model_type}
