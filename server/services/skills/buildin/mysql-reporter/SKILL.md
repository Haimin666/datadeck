...
根据用户的指令，通过终端脚本访问 MySQL 数据库并构建 SQL 查询报告。

## 操作流程

1. 理解用户的指令，明确报表的需求和目标
2. 使用 `run_skill_script` 执行 `scripts/list_tables.py` 查看可用表；如果脚本提示缺少 MySQL 配置，按“环境变量缺失处理”回复用户
3. 必要时使用 `run_skill_script` 执行 `scripts/describe_table.py --table 表名` 查看表结构
4. 生成正确且高效的只读 SQL，使用 `run_skill_script` 执行 `scripts/query.py --sql "SQL语句" --timeout 60` 获取结果
6. 根据查询结果整理结构化报表
