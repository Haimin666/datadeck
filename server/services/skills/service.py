...
from server.services.mcp.service import get_enabled_mcp_server_slugs
from server.services.skills.repository import SkillRepository
from server.dataroot import (
    get_runtime_dir,
    get_skill_data_dir,
    get_skill_projection_dir,
)
from server.permissions import ResourcePermission, normalize_permission_config, resolve_skill_permission
from server.models import Skill, User
from datadeck import logger
from server.utils.paths import ensure_within_root
...
async def refresh_user_skill_projection_async(uid: str) -> dict[str, str]:
    ...
    from server.db import session_context as _pg_session
...
def get_builtin_skill_specs() -> list[Any]:
    from server.services.skills.buildin import BUILTIN_SKILLS
...
def get_personal_skills_root_dir(uid: str) -> Path:
    ...
    from server.workspace.paths import user_workspace_dir
...
async def enable_personal_skills_for_agent_config(
    ...
    *,
    ...
    datadeck 适配：threads/agents 表与 Yuxi Conversation/Agent 语义一致
    （threads.id=thread_id、threads.uid、threads.agent_id=agents.slug）。
    datadeck agents 无个人归属（created_by），全部视为可配置，跳过归属校验。
    """
    from sqlalchemy import select as _select

    from server.models import Agent, Thread

    result = await db.execute(_select(Thread).where(Thread.id == thread_id))
    thread = result.scalar_one_or_none()
    if not thread or str(thread.uid) != str(uid):
        return False
    result = await db.execute(_select(Agent).where(Agent.slug == thread.agent_id))
...
async def prepare_remote_skill_install(
    ...
    *,
    ...
    from server.services.skills.remote_install import prepare_remote_skills_batch
...
async def init_builtin_skills(db: AsyncSession, *, created_by: str = "system") -> list[Skill]:
    ...
    for spec in list_builtin_skill_specs():
        ...
        if existing:
            existing.share_config = BUILTIN_SKILL_SHARE_CONFIG.copy()
