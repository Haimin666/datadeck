...
from server.services.runtime_paths import is_runtime_path, runtime_path_for_workdir_scope
from server.services.file_preview import render_file_preview
from server.services.workdir_service import AuthorizedWorkdir, resolve_authorized_workdir
...
from server.utils.upload_utils import write_upload_to_path
from server.workspace.errors import FileTransferLimitError
...
async def _download_to_temp(workdir, path: str, max_bytes: int) -> str:
    descriptor, temp_path = tempfile.mkstemp(prefix="datadeck-viewer-", suffix=PurePosixPath(path).suffix)
    os.close(descriptor)
    try:
...
async def upload_viewer_files(*, thread_id: str, parent_path: str, files: list[UploadFile], current_user, db) -> dict:
    ...
    for upload, file_name in zip(files, file_names, strict=True):
        descriptor, temp_path = tempfile.mkstemp(prefix="datadeck-viewer-upload-")
        os.close(descriptor)
        try:
