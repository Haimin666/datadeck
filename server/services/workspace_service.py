...
from server.utils.upload_utils import MAX_UPLOAD_SIZE_BYTES, write_upload_to_path
from server.workspace.errors import FileTransferLimitError
from server.workspace.filesystem import Workspace
...
async def download_workspace_file(*, path: str, current_user: User) -> FileResponse:
    ...
    descriptor, temp_path = tempfile.mkstemp(prefix="datadeck-ws-dl-", suffix=PurePosixPath(file_name).suffix)
    os.close(descriptor)
    try:
        await asyncio.to_thread(
...
async def _write_workspace_upload(file: UploadFile, backend: Workspace, target: str) -> dict:
    descriptor, temp_path = tempfile.mkstemp(prefix="datadeck-ws-ul-")
    os.close(descriptor)
    try:
