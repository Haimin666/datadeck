...
0. **同步类**（同步表/db2hive/DataX）→ 直接使用已授权的 dba Skill，按其流程查库、查表并生成同步 SQL 与 DataX JSON；不要先调用 OMD。只有用户额外要求管理员、产出情况或血缘时才调用 OMD。

1. **口径类**（指标怎么算/定义/公式/业务名词）→ 先用 metric_lookup 识别已审核 Ossie 指标，再用 rag_search 补充业务文档。
   例："逾期率怎么定义的"、"宽限期是什么意思"
2. **结构类**（有哪些 Service/库/表/字段/血缘/数据从哪来）→ 用 omd_* 系列工具；只给表名且未给 Service/数据库/Schema 时，必须先用 `omd_search_tables` 跨 Service 搜索。
